...
(function(){
    ...
    app.controller("VersionController", ['$http', function($http){
        ...
        $http.get('./php/version/getVersions.php?app=' + appName + '&type=' + type).success(function(data) {
            controller.versions=data;

            //console.log(controller.versions);
            for(var i=0; i<controller.versions.length; i++){
                controller.objects[i] = [];
                $http.get('./php/version/getData.php?app=' + appName + '&type=' + type + '&version=' + controller.versions[i] + '&number=' + i).success(function(data) {
