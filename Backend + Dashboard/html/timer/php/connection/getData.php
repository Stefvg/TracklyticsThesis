...
$query = "SELECT name, AVG(durationTime) AS durationTime, SUM(numberOfMeasurements) AS numberOfMeasurements FROM TimerAgg_View WHERE type='$type' AND appName='$app'  AND connectionType='$connectionType'";
$result = mysql_query($query);
$aggArray = array();
if (!$result) {
...
$query = "SELECT name,AVG(durationTime) AS durationTime FROM Timer_View WHERE type='$type' AND appName='$app'  AND connectionType='$connectionType'";
$result = mysql_query($query);
if (!$result) {
    die('Invalid query: ' . mysql_error());
}
