...
if($min && $max) {
    $query = "SELECT DISTINCT date FROM MeterAgg_View WHERE type='$type' AND device ='$device' AND version='$version' AND DATE(date) <='$max' AND DATE(date) >= '$min' AND appName='$app' ORDER BY date ASC";
}else {
    $query = "SELECT DISTINCT date FROM MeterAgg_View WHERE type='$type' AND device ='$device' AND version='$version' AND appName='$app' ORDER BY date ASC";
}
$result = mysql_query($query);
if (!$result) {
    die('Invalid query: ' . mysql_error());
